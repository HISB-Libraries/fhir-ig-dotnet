...
public class FlatObjectCMELE : FlatObject
{
    ...
    {
        List<Resource> resources = composition.VdorComposition().GetSectionByCode(VdorCustomCs.Weapons);
        foreach (Resource resource in resources)
        {
            if (resource is Observation weaponObs)
            {
                if (!weaponObs.Code.IsNullOrEmpty())
                {
                    ...
                    {
                        if (VdorWeaponsCategoryVs.WeaponType.Coding[0].Code.Equals(codings[0].Code))
                        {
                            return weaponObs;
                        }
                    }
    ...
    public override void MapToNVDRS(Bundle bundle)
    {
        if (bundle.Meta != null && bundle.Meta.Profile != null && bundle.Meta.Profile.Any())
        {
            ...
            List<Resource> circumstanceResources = composition.VdorComposition().GetSectionByCode(VdorCustomCs.Circumstances);

            // get the list of entries in the Injury and Death section
            List<Resource> injuryAndDeathResources = composition.VdorComposition().GetSectionByCode(VdorCustomCs.InjuryAndDeath);

            // get the list of entries in the Demographics section
            List<Resource> deomgraphicsResources = composition.VdorComposition().GetSectionByCode(VdorCustomCs.Demographics);
            ...
            foreach (JsonNode? data in DataArray)
            {
                ...
                {
                    if (composition.VdorComposition().ForceNewRecord)
                    {
                        data["value"] = "Y";
                    }
                    else
                ...
                {
                    if (composition.VdorComposition().OverwriteConflicts)
                    {
                        data["value"] = "Y";
                    }
                    // else
                ...
                {
                    SetInjuryAndDeathYNUnk(data, injuryAndDeathResources, VdorCustomCs.EmsAtScene);
                }
                else if ("VictimInCustody".Equals(data!["name"]!.GetValue<string>())) // 111
                {
                    SetInjuryAndDeathYNUnk(data, injuryAndDeathResources, VdorCustomCs.VictimInCustodyWhenInjured);
                }
                ...
                {
                    SetDemographicsYNUnk(data, deomgraphicsResources, VdorCustomCs.HomelessAtDeath);
                }
                ...
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToHead);
                }
                else if ("WoundToFace".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToFace);
                }
                else if ("WoundToNeck".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToNeck);
                }
                else if ("WoundToUpperExtremity".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToUpperExtremity);
                }
                else if ("WoundToSpine".Equals(data!["name"]!.GetValue<string>())) // 429
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToSpine);
                }
                else if ("WoundToThorax".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToThorax);
                }
                else if ("WoundToAbdomen".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToAbdomen);
                }
                else if ("WoundToLowerExtremity".Equals(data!["name"]!.GetValue<string>()))
                {
                    SetWoundResponse(data, injuryAndDeathResources, VdorCustomCs.WoundToLowerExtremity);
                }
                ...
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.DeathAbuse);
                }
                ...
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.CurrentDepressedMood);
                }
                else if ("DriveByShootingCME".Equals(data!["name"]!.GetValue<string>())) // 724
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.DriveByShooting);
                }
                else if ("DrugInvolvementCME".Equals(data!["name"]!.GetValue<string>())) // 725
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.DrugInvolvement);
                }
                else if ("GangRelatedCME".Equals(data!["name"]!.GetValue<string>())) // 730
                {
                    foreach (Resource resource in circumstanceResources)
                    {
                        if (resource is Observation obs)
                        {
                            if (VdorCustomCs.GangRelated.CodingExist(obs.Code))
                            {
                                if (obs.Value is CodeableConcept cc)
                                {
                ...
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.PlayWithFirearm);
                }
                else if ("RandomViolenceCME".Equals(data!["name"]!.GetValue<string>())) // 824
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.RandomViolence);
                }
                else if ("SchoolProblemCME".Equals(data!["name"]!.GetValue<string>())) // 828
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.SchoolProblem);
                }
                else if ("SuicideAttemptHistoryCME".Equals(data!["name"]!.GetValue<string>())) // 832
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.HistoryOfSuicideAttempts);
                }
                ...
                {
                    SetCircumstanceYN(data, circumstanceResources, VdorCustomCs.SelfHarm);
                }
            }
        }
